...
class TestVerifyIntegrity:
    def test_clean_state_returns_0(self, mini_repo: Path) -> None:
        ...
        r = _run(
            [sys.executable, "-m", "kms.scripts.verify_integrity", "--config", cfg],
            mini_repo,
        )
    ...
    def test_orphan_inbox_file_returns_1(self, mini_repo: Path) -> None:
        (mini_repo / "vault" / "00_Inbox" / "orphan.txt").write_text(
            "no db entry", encoding="utf-8"
        )
        cfg = str(mini_repo / "kms" / "config" / "config.yaml")
        r = _run(
            [sys.executable, "-m", "kms.scripts.verify_integrity", "--config", cfg],
...
class TestDailyReport:
    ...
    def test_writes_report_file(self, mini_repo: Path) -> None:
        cfg = str(mini_repo / "kms" / "config" / "config.yaml")
        r = _run(
            [sys.executable, "-m", "kms.scripts.daily_report", "--config", cfg],
            mini_repo,
        )
        assert r.returncode == 0
        reports = list(
            (mini_repo / "vault" / "00_Admin" / "reports").glob("daily-*.txt")
        )
    ...
    def test_creates_audit_entry(self, mini_repo: Path) -> None:
        ...
        row = conn.execute(
            "SELECT action FROM audit_log WHERE action = 'daily_report'"
        ).fetchone()
        conn.close()
...
class TestGenerateSourceNote:
    def test_dry_run_does_not_create_file(self, mini_repo: Path) -> None:
        ...
        r = _run(
            [
                sys.executable,
                "-m",
                "kms.scripts.generate_source_note",
                "--title",
                "Dry Run Test",
                "--dry-run",
                "--config",
    ...
    def test_creates_audit_entry(self, mini_repo: Path) -> None:
        ...
        conn = sqlite3.connect(mini_repo / "kms" / "data" / "state.db")
        row = conn.execute(
            "SELECT action FROM audit_log WHERE action = 'generate_source_note'"
        ).fetchone()
        conn.close()
        assert row is not None
...
class TestGenerateDashboard:
    def test_creates_dashboard_file(self, mini_repo: Path) -> None:
        cfg = str(mini_repo / "kms" / "config" / "config.yaml")
        r = _run(
            [sys.executable, "-m", "kms.scripts.generate_dashboard", "--config", cfg],
            mini_repo,
        )
    ...
    def test_dashboard_shows_inbox_count(self, mini_repo: Path) -> None:
        (mini_repo / "vault" / "00_Inbox" / "file1.md").write_text(
            "test", encoding="utf-8"
        )
        ...
        assert r.returncode == 0, r.stderr + r.stdout
        content = (mini_repo / "vault" / "00_Admin" / "dashboard.md").read_text(
            encoding="utf-8"
        )
        assert "2" in content
...
class TestStatus:
    def test_empty_db_returns_0(self, mini_repo: Path) -> None:
        cfg = str(mini_repo / "kms" / "config" / "config.yaml")
        r = _run(
            [sys.executable, "-m", "kms.scripts.status", "--config", cfg], mini_repo
        )
        assert r.returncode == 0
    ...
    def test_json_output_parseable(self, mini_repo: Path) -> None:
        ...
        (mini_repo / "vault" / "00_Inbox" / "test.txt").write_text(
...
class TestConvertConversation:
    def test_dry_run_does_not_create_file(self, mini_repo: Path) -> None:
        ...
        r = _run(
            [
                sys.executable,
                "-m",
                "kms.scripts.convert_conversation",
                "--input",
                str(conv_file),
                "--title",
                "Dry Run Conv",
    ...
    def test_creates_audit_entry(self, mini_repo: Path) -> None:
        ...
        conn = sqlite3.connect(mini_repo / "kms" / "data" / "state.db")
        row = conn.execute(
            "SELECT action FROM audit_log WHERE action = 'convert_conversation'"
        ).fetchone()
        conn.close()
        assert row is not None
