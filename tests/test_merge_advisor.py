...
PERMANENT_NOTES = (
    Path(__file__).resolve().parents[1] / "example-vault" / "30_Permanent-Notes"
)
...
def test_analyze_raii_overlap(perm_notes: Path) -> None:
    ...
    assert any(
        "raii" in (m.get("title") or "").lower()
        or "raii" in (m.get("note_path") or "").lower()
...
def test_analyze_returns_prompt_stub(perm_notes: Path) -> None:
    ...
    assert (
        "inbox" in r.llm_prompt_markdown.lower()
        or "Nowy tekst" in r.llm_prompt_markdown
    )
